...
# Script to move json_rpc connector submodule to a new GitHub repository
# Uses GitHub CLI (gh) to create the new repo

set -e  # Exit on error

SUBMODULE_PATH="submodules/active_data_flow-connector-json_rpc"
NEW_REPO_NAME="active_data_flow-connector-json_rpc"
TEMP_DIR="/tmp/${NEW_REPO_NAME}"

echo "=== Moving JSON-RPC Connector to New Repository ==="
echo ""
...
echo "   git rm -f $SUBMODULE_PATH"
echo "   rm -rf .git/modules/$SUBMODULE_PATH"
echo "   git commit -m 'Remove json_rpc connector submodule'"
