...
# Script to move source connector submodule to a new GitHub repository
# Uses GitHub CLI (gh) to create the new repo

set -e  # Exit on error

SUBMODULE_PATH="submodules/active_data_flow-connector-source-json_rpc"
NEW_REPO_NAME="active_data_flow-connector-source-json_rpc"
TEMP_DIR="/tmp/${NEW_REPO_NAME}"

echo "=== Moving Source Connector to New Repository ==="
echo ""
...
echo "   git rm -f $SUBMODULE_PATH"
echo "   rm -rf .git/modules/$SUBMODULE_PATH"
echo "   git commit -m 'Remove source connector submodule'"
